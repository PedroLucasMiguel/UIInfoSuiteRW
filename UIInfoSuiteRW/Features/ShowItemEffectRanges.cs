...
namespace UIInfoSuiteRW.Features
{
  internal class ShowItemEffectRanges : IFeature
  {
    ...
    #region Lifecycle
    public ShowItemEffectRanges(IModHelper helper, ModConfig config)
    {
      _helper = helper;
      _config = config;
    }
    ...
    private void UpdateEffectiveArea()
    {
      ...
      // Maybe refactor this
      if (_config.ShowPlacedObjectRange.IsDown() && Game1.currentLocation != null)
      {
        ...
        if (mouseItem != null)
        {
          string itemName = mouseItem.Name;
          ...
          else if (itemName.IndexOf("sprinkler", StringComparison.OrdinalIgnoreCase) >= 0)
          {
            ...
            */
            ...
            // Move tiles to 0, 0 and then offset by the correct tile.
            IEnumerable<Vector2> unplacedSprinklerTiles = mouseItem.GetSprinklerTiles();
            if (mouseItem.TileLocation != validTile)
            {
              unplacedSprinklerTiles = unplacedSprinklerTiles.Select(tile => tile - mouseItem.TileLocation + validTile);
            }
